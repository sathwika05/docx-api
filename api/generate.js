...
function projectTitle(text, githubUrl, demoUrl) {
  const noBorder = { style: BorderStyle.NONE, size: 0, color: "FFFFFF" };
  const borders = { top: noBorder, bottom: noBorder, left: noBorder, right: noBorder };

  const leftChildren = [
    new TextRun({ text, font: CALIBRI, size: PT(11), bold: true, color: BLACK })
  ];
  if (githubUrl) {
    leftChildren.push(new TextRun({ text: "  ", font: CALIBRI, size: PT(11), color: BLACK }));
    leftChildren.push(new ExternalHyperlink({
      link: githubUrl,
      children: [new TextRun({ text: "[GitHub]", font: CALIBRI, size: PT(10), color: "0563C1", underline: {} })]
    }));
  }

  const rightChildren = demoUrl ? [
    new ExternalHyperlink({
      link: demoUrl,
      children: [new TextRun({ text: "Demo", font: CALIBRI, size: PT(10), bold: true, color: "0563C1", underline: {} })]
    })
      ...
      new TableRow({
        children: [
          new TableCell({
            ...
            verticalAlign: VerticalAlign.TOP,
            margins: { top: 0, bottom: 0, left: 0, right: 0 },
            children: [new Paragraph({ spacing: { before: 60, after: 0 }, children: leftChildren })]
          }),
          new TableCell({
            borders,
            width: { size: 2520, type: WidthType.DXA },
            verticalAlign: VerticalAlign.TOP,
            margins: { top: 0, bottom: 0, left: 0, right: 0 },
            children: [new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { before: 60, after: 0 }, children: rightChildren })]
          })
        ]
      })
    ]
...
function buildDoc(d) {
  ...
  children.push(new Paragraph({
    ...
    children: [
      new ExternalHyperlink({
        link: "http://sathwikap.com/",
        children: [new TextRun({ text: "sathwikap.com", font: CALIBRI, size: PT(11), color: "0563C1", underline: {} })]
      }),
      new TextRun({ text: " | ", font: CALIBRI, size: PT(11), color: BLACK }),
      new ExternalHyperlink({
        link: "http://linkedin.com/in/sathwikaparshaboina/",
        children: [new TextRun({ text: "linkedin.com/in/sathwikaparshaboina/", font: CALIBRI, size: PT(11), color: "0563C1", underline: {} })]
      }),
    ]
  }));
  ...
  children.push(projectTitle("AI-Vid", "https://github.com/sathwika05/AIVid", "https://aivid-frontend.onrender.com"));
  safeArray(d.projects?.ai_vid_bullets).forEach(b => children.push(bulletItem(b)));
  children.push(projectTitle("AI-Appointment-Intake-Workflow", "https://github.com/sathwika05/AI-Appointment-Intake-Workflow", "https://ai-appointment-intake-workflow-frontend.onrender.com/admin"));
  safeArray(d.projects?.ai_appointment_bullets).forEach(b => children.push(bulletItem(b)));
  children.push(projectTitle("AI Task Management System", "https://github.com/sathwika05/AI-Task-Management-System", null));
  safeArray(d.projects?.ai_task_bullets).forEach(b => children.push(bulletItem(b)));
  children.push(projectTitle("AI DocCrawler", "https://github.com/sathwika05/AI-DocCrawler", null));
